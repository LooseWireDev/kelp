    ...
    private fun LightResult.Error.toTideError(): TideError =
        TideError.decodeOrNull(extra) ?: when (code) {
            ...
            LightResult.ErrorCode.InvalidParameters ->
                TideError(TideErrorCategory.Protocol, "Kelp could not understand the server request.")

            LightResult.ErrorCode.Removed ->
                TideError(TideErrorCategory.Unavailable, "This Kelp feature is no longer available.")

            LightResult.ErrorCode.Unknown ->
                TideError(TideErrorCategory.Unavailable, "Could not connect to the Kelp server.")
        }
}
