...
object TideServiceMethods {
    ...
    private var loginActivityComponent = "com.loosewire.tide/.server.LoginActivity"
    ...
    fun dispatch(methodId: String, payload: String?): LightResult<String> = try {
        when (methodId) {
            ...
            TideRemoteMethod.ControlPlayback.id -> {
                ...
                TideErrorCategory.Protocol,
                "Tide does not support this server request.",
                LightResult.ErrorCode.InvalidParameters,
            )
        }
    } catch (error: SerializationException) {
        logger.warning(
            "Invalid Tide RPC payload for method $methodId: ${error.message ?: error.javaClass.simpleName}",
        )
        failure(
            TideErrorCategory.Protocol,
            "Tide received an invalid request.",
            LightResult.ErrorCode.InvalidParameters,
        )
    } catch (error: Exception) {
        logger.severe("Tide RPC failed for method $methodId: ${error.javaClass.simpleName}")
        failure(TideErrorCategory.Unavailable, "The Tide server could not complete the request.")
    }
